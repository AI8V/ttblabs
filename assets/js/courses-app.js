...
(function () {
  ...
  function buildCard(course, idx) {
    ...
    var instructor = U.el('span', { className: 'course-card-instructor' }, [
      U.el('i', { className: 'bi bi-person-fill me-1', aria: { hidden: 'true' } }), course.instructor
    ]);
    ...
    // Footer
    var priceEl = U.el('span', { className: 'course-card-price' + (course.price === 0 ? ' free' : ''), textContent: price });
  ...
  function buildFiltersDOM() {
    ...
    [{ v: 0, t: 'All Ratings' }, { v: 1, t: '★ & up' }, { v: 2, t: '★★ & up' }, { v: 3, t: '★★★ & up' }, { v: 4, t: '★★★★ & up' }].forEach(function (o) {
      var id = 'rating-' + o.v;
    ...
    // Buttons
    var applyBtn = U.el('button', { className: 'filter-btn-apply', type: 'button', textContent: 'Apply Filters', events: { click: function () { closeMobile(); render(true); } } });
    var resetBtn = U.el('button', { className: 'filter-btn-reset', type: 'button', textContent: 'Reset', events: { click: function () { resetAll(); closeMobile(); } } });
  ...
  function closeMobile() { if (!DOM.offcanvas) return; try { var i = bootstrap.Offcanvas.getInstance(DOM.offcanvas); if (i) i.hide(); } catch (e) {} }
  ...
  function buildPagination(total) {
    ...
    function mkPage(label, page, opts) {
      ...
      a.addEventListener('click', function (e) { e.preventDefault(); if (!opts.disabled && state.currentPage !== page) { state.currentPage = page; render(false); if (DOM.grid) DOM.grid.scrollIntoView({ behavior: 'smooth', block: 'start' }); } });
      li.appendChild(a);
      return li;
    }
    ...
    var d = 2, range = [];
    for (var i = 1; i <= pages; i++) { if (i === 1 || i === pages || (i >= state.currentPage - d && i <= state.currentPage + d)) range.push(i); }
    var last = 0;
    range.forEach(function (p) {
      if (p - last > 1) { var ell = U.el('li', { className: 'page-item disabled' }); ell.appendChild(U.el('span', { className: 'page-link', textContent: '…' })); frag.appendChild(ell); }
  ...
  function buildSort() {
    ...
    SORT_OPTIONS.forEach(function (o) {
      var a = U.el('a', { className: 'dropdown-item' + (o.key === state.currentSort ? ' active' : ''), href: '#', textContent: o.label, dataset: { sortKey: o.key } });
      a.addEventListener('click', function (e) { e.preventDefault(); state.currentSort = o.key; updateSortLabel(); highlightSort(); render(true); });
  ...
  function highlightSort() {
    if (!DOM.sortMenu) return;
    U.qsa('.dropdown-item', DOM.sortMenu).forEach(function (i) { i.classList.toggle('active', i.dataset.sortKey === state.currentSort); });
  }
  ...
  function readURL() {
    ...
    if (cats.length && state.filtersEl) U.qsa('input[data-filter="category"]', state.filtersEl).forEach(function (cb) { cb.checked = cats.indexOf(cb.value) !== -1; });
    var rt = parseInt(p.get('rating'), 10);
    if (VALID_RATINGS.indexOf(rt) !== -1 && state.filtersEl) { var ri = U.qs('input[name="rating-filter"][value="' + rt + '"]', state.filtersEl); if (ri) ri.checked = true; }
    var lv = p.get('level');
    if (VALID_LEVELS.indexOf(lv) !== -1 && state.filtersEl) { var li = U.qs('input[name="level-filter"][value="' + lv + '"]', state.filtersEl); if (li) li.checked = true; }
  ...
  function updateSchema(courses) {
    ...
    var schema = { "@context": "https://schema.org", "@type": "ItemList", "name": "Online Courses", "numberOfItems": courses.length, "itemListElement": courses.map(function (c, i) { return { "@type": "ListItem", "position": i + 1, "item": { "@type": "Course", "url": location.origin + '/course/course-details/index.html?id=' + c.id, "name": c.title, "description": c.description, "provider": { "@type": "Organization", "name": "Your Brand Name" } } }; }) };
    var s = document.createElement('script'); s.type = 'application/ld+json'; s.id = 'jsonld-courses'; s.textContent = JSON.stringify(schema);
  ...
  function render(resetPage) {
    ...
    var total = sorted.length, start = (state.currentPage - 1) * CONFIG.CARDS_PER_PAGE, end = Math.min(start + CONFIG.CARDS_PER_PAGE, total);
    var page = sorted.slice(start, end);

    if (DOM.grid) {
      while (DOM.grid.firstChild) DOM.grid.removeChild(DOM.grid.firstChild);
      if (!page.length) DOM.grid.appendChild(buildEmpty());
      else { var frag = document.createDocumentFragment(); page.forEach(function (c, i) { frag.appendChild(buildCard(c, i)); }); DOM.grid.appendChild(frag); }
    }
    if (DOM.results) DOM.results.textContent = 'Showing ' + (total ? start + 1 : 0) + '–' + end + ' of ' + total + ' results';
  ...
  function resetAll() {
    ...
    updateSortLabel(); highlightSort(); render(true);
  }

  // ── Events ──

  function bindEvents() {
    if (DOM.search) DOM.search.addEventListener('input', U.debounce(function () { render(true); }, CONFIG.DEBOUNCE_DELAY));
    if (state.filtersEl) state.filtersEl.addEventListener('change', function (e) {
